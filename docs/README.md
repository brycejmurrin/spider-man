...
| [`MISSIONS.md`](research/MISSIONS.md) | What the player does when NOT swinging, and what a one-developer procedural clone should build instead. The activity taxonomy with counts, the surfacing/spawn research, the token economy (it gates almost nothing), the five-of-34 traversal skills, and a per-activity verdict: seedable free / needs new generator data / skip. Ends with the minimum viable content loop and why a bare self-ghost is not it. | `docs/PLAN.md` §5 |
